...
// - store_products 테이블의 메뉴 / 상품 정보를 함께 표시
...
export default function StoreMiniHomePage({ mode = "travel" }) {
    ...
    async function loadStoreMiniHome() {
      ...
      try {
        ...
        if (productResult.error) {
          console.warn("[StoreMiniHomePage] store_products 조회 확인 필요:", productResult.error);
        }

        if (eventResult.error) {
          console.warn("[StoreMiniHomePage] store_events 조회 확인 필요:", eventResult.error);
        }
  ...
  function handlePrimaryAction() {
    if (mode === "delivery") {
      setNoticeMessage(
        "배달 주문 기능은 다음 단계에서 장바구니 / 주문서와 연결할 예정입니다."
      );
      return;
    }

    setNoticeMessage(
      "방문 예약 기능은 다음 단계에서 감성여행2 여행 일정과 연결할 예정입니다."
    );
  }
  ...
  function handleSecondaryAction() {
    if (mode === "delivery") {
      setNoticeMessage(
        "메뉴 담기 기능은 다음 단계에서 장바구니와 연결할 예정입니다."
      );
      return;
    }

    setNoticeMessage(
      "여행 코스 담기 기능은 다음 단계에서 감성여행2 코스/버킷과 연결할 예정입니다."
    );
  }
    ...
    <main className={`store-mini-home-page ${modeInfo.modeClass}`}>
      <section className="store-mini-hero">
        ...
        <div className="store-mini-hero-content">
          ...
          {noticeMessage ? (
            <p className="store-mini-notice">{noticeMessage}</p>
          ) : null}
        </div>
      </section>
      ...
      <section className="store-mini-section">
        <div className="store-mini-section-head">
          <div>
            <span className="store-mini-label">MENU / PRODUCT</span>
            <h2>{mode === "delivery" ? "배달 메뉴" : "방문 전 확인할 메뉴"}</h2>
          </div>

          <p>
            {mode === "delivery"
              ? "감성배달에서는 이 메뉴가 배달 주문으로 연결됩니다."
              : "감성여행2에서는 이 메뉴가 방문 예약 전 확인 정보로 표시됩니다."}
          </p>
        </div>
        ...
        {products.length > 0 ? (
            ...
            {products.map((product) => {
              ...
              return (
                <article key={product.id} className="store-product-card">
                  {productImageUrl ? (
                    <img src={productImageUrl} alt={getProductName(product)} />
                  ) : (
          ...
          <div className="store-mini-ready-box">
            ...
            <p>
              사장님이 휴대폰에서 메뉴와 상품을 등록하면 이곳에 표시됩니다.
              {modeInfo.emptyAction}
            </p>
          </div>
        )}
      </section>
